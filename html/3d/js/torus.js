/*********
 * torus.js
 * A torus given  number of knots, outer radius, inner radius, start and end angles
 * B. Volegov built upon cylinder.js, segmentedCylinder.js, knottedCylinder.js
 * February 2016
 ***********/
...
function createTorus(n, nbrKnots, outerRadius, innerRadius, startAngle, endAngle) {
    var nbrSegments = nbrKnots - 1;
    var angleInc = (endAngle - startAngle) / nbrSegments;

    var heights = [];
    var scales = [];

    for(var i = 0, angle = startAngle; i < nbrKnots; i++, angle+=angleInc) {
        scales.push(outerRadius + innerRadius * Math.cos(angle));
        heights.push(innerRadius * Math.sin(angle));
    }

    return createKnottedCylinder(n, heights, scales, false, false)
}
...
function createScene() {
    ...
    var mesh = createTorus(n, nbrKnots, outerRadius, innerRadius, startAngle, endAngle);
